...
const g = ({
  ...
  const {
    colors: a
  } = styledComponents.useTheme(), w = n && n in a ? a[n] : n, e = t && t in a ? a[t] : t;
...
react.forwardRef(V2);
const PLUGIN_ID = "dashboard-widgets";
const index = {
  register(app) {
    app.addMenuLink({
      ...
      position: 0,
      Component: async () => {
        const { DashboardPage } = await Promise.resolve().then(() => require("./DashboardPage-DWNVZog8.js"));
